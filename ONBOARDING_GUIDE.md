...
**最后更新**: 2026-03-06
**项目状态**: 开发中，核心功能已实现
**预计阅读时间**: 2-3 小时（完整阅读），30 分钟（快速上手），5 分钟（Codex 即时启动）

---

## ⚡ Codex 即时启动（5 分钟）

> 如果你是 Codex 或其他 AI 代码执行器，请**先读这一节**，再读其他内容。

### 当前迭代任务

最新的 Codex 执行包（Execution Brief）在此：

```
Docs/CODEX_EXECUTION_BRIEF_2026-03-06_iter-001.md
```

**立即行动**：
1. 阅读上述 Execution Brief（约 3 分钟）
2. 按照 Brief 中的"上下文加载顺序"逐个读文件
3. 按照"Implementation Steps"执行，不要跳步
4. 每步完成后更新 Session Log

### 项目当前状态（截至 2026-03-06 17:30）

| 模块 | 状态 | 备注 |
|------|------|------|
| 文字/语音记录 | ✅ 可用 | 语音转写为占位 |
| AI 清理文本（Clean） | ✅ 可用 | 接入 OpenAI/后端 |
| AI 拆分（Atomization） | ✅ 可用 | 线程修复已完成 |
| 标签建议 | ✅ 可用 | 1 visible + 5 hidden |
| 源文本追溯（来源高亮） | ⚠️ 数据就绪，UI 未实现 | **本次迭代 Goal 1** |
| 单元测试 | ⚠️ 覆盖率为零 | **本次迭代 Goal 2** |
| 语音转写（真实实现） | ❌ 占位 | P1 待排期 |
| 隐私脱敏 | ✅ 已实现（5 模式） | 待手动验证 |

### 绝对禁区（Codex 不得修改）

- `PersistenceController.swift` — CoreData schema 已稳定
- `AtomizationCoordinator.swift` — 线程模型刚修复，不要动 @MainActor 相关
- `AIService.swift` — AI 调用逻辑
- `*.xcodeproj` — Xcode 项目配置
- `Rules/`、`Skills/`、`Docs/` — 规范文档（由规范负责人维护）

---

## 📋 目录

1. [Codex 即时启动](#codex-即时启动5-分钟)（新增 v1.1）
2. [快速上手路径](#快速上手路径-30-分钟)
3. [完整阅读路径](#完整阅读路径-2-3-小时)
4. [项目结构概览](#项目结构概览)
5. [关键文档索引](#关键文档索引)
6. [开发工作流程](#开发工作流程)
7. [当前状态与待办事项](#当前状态与待办事项)
8. [常见问题解答](#常见问题解答)
