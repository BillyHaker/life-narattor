...
final class OpenAIService: AIService {
    ...
    func suggestTags(atomization: AtomizeResult, tagLibrary: TagLibrary) async throws -> TagSuggestionResult {
        ...
        let output = try await requestJSON(
            ...
            instructions: """
            Return JSON only.
            Suggest at most one visible tag.
            ...
            Prefer reusing an existing visible tag from the provided tag library whenever there is a close semantic match.
            Only suggest a new visible tag when no existing visible tag is close enough.
