...
默认情况下，记录内容以本机保存为主，并可通过用户自己的 Apple iCloud 私有数据库同步文字记录、转写、整理结果、拆分结构和标签。Life Narrator 服务器不会保存完整记录作为长期内容主库。只有在语音转写、助手对话、记录整理、AI 回顾等需要联网处理的功能中，才会把必要内容通过产品后台代理发送给模型服务处理。应用不会把上游模型服务密钥放进客户端或暴露给测试用户。
...
隐私边界如下：
1. 记录内容以本机保存为主，并可通过用户自己的 Apple iCloud 私有数据库同步核心文字与结构化数据。
2. Life Narrator 服务器不会保存完整记录作为长期内容主库。
3. 当前版本不承诺跨设备恢复原始录音文件。
4. 只有在语音转写、助手对话、记录整理和 AI 回顾等需要联网处理的功能中，应用才会通过产品后台代理调用 AI 能力。
5. 应用不会向终端用户暴露上游 AI 服务提供方密钥，也不会在客户端内置测试用模型密钥。
