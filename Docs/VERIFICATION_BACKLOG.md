...
### VRF-046
- feature: 底部主导航贴底和收窄
- description: `记录 / 时间线 / AI 回顾` 主导航应比上一版更贴近底部，胶囊高度略收窄，但仍保持宽度、可点击性和三层底部结构。输入栏不能再次被遮挡。
- type: human-visual
- added: 2026-05-08
- status: pending
- notes: 验证路径为启动 App -> 记录页 -> 检查主导航与 Home Indicator 的距离；确认 `记录 / 助手`、输入栏、主导航三层都可见；在 Pro Max 和小屏设备上确认主导航不显得漂浮，也不压迫底部安全区。

### VRF-047
- feature: 记录拆分因果保守保护
- description: 拆分不应把时间相邻或同段出现的事实改写为确定因果；只有用户原文明确表达因果时，才可以保留确定因果。
- type: human-behavior
- added: 2026-05-09
- status: pending
